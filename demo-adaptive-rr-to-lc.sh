...
echo -e "${BLUE}├─────────────────────────────────────────────────────────────────┤${NC}"
echo -e "${BLUE}│${NC} 3001  │ ${GREEN}Fast${NC}     │ 50ms     │ 0%         │ Baseline         ${BLUE}│${NC}"
echo -e "${BLUE}│${NC} 3002  │ ${RED}Slow${NC}     │ 800ms    │ 0%         │ High Latency     ${BLUE}│${NC}"
echo -e "${BLUE}│${NC} 3003  │ ${YELLOW}Faulty${NC}   │ 400ms    │ 20%        │ High Lat + Err   ${BLUE}│${NC}"
echo -e "${BLUE}└─────────────────────────────────────────────────────────────────┘${NC}"
...
echo -e "      - High Latency: ${YELLOW}>300ms${NC}"
...
echo -e "${YELLOW}Target: Generate enough requests to trigger high latency detection${NC}"
echo -e "${YELLOW}Expected: Switch from round_robin → least_connections (need majority >300ms)${NC}"
echo -e "${YELLOW}Note: Decision engine evaluates every 5 seconds${NC}"
echo ""
sleep 3

# Generate traffic in background (100 requests, 0.3s interval = ~30 seconds)
echo -e "${CYAN}Sending 100 requests (0.3s interval, ~30 seconds)...${NC}"
(
    for i in {1..100}; do
